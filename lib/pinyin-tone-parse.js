const { UntonedWordError } = require('./errors')
 ...
 * @property {boolean} allowUntoned Allow words without exiplicit tone number (like `de`).
 */
 ...
 * @throws {UntonedWordError} Found a word without explicit tone number.
 */
function parse(text, options) {
    ...
    for (let i = 0, len = t.length; i < len; i++) {
        ...
        if (isLetter(c)) {
            ...
            if (startIdx !== undefined) {
                if (options.allowUntoned === false) {
                    // close word
                    parsed.push([t.slice(startIdx, i), 5])
                    startIdx = undefined
                } else {
                    throw new UntonedWordError(t.slice(startIdx, i))
                }
            }
            parsed.push(' ')
